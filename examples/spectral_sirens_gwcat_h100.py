#!/usr/bin/env python3
"""Run/benchmark a catalog-free spectral-siren closure with darksirens-core.

Inputs may be given explicitly as ``PE SELECTION`` or through ``--mock-dir``
created by ``generate_mock_dark_siren_closure.py``.  In mock mode the manifest
truth and generator provenance are copied into the timing JSON so the spectral,
complete-catalog and incomplete-catalog runs can be compared on exactly the
same GW realization.

Examples
--------
Benchmark the generated closure realization only::

    python examples/spectral_sirens_gwcat_h100.py \
        --mock-dir data/mock_closure --require-gpu \
        --fixed-population legacy --benchmark-only --likelihood-evals 100 \
        --output-prefix perf/spectral_fixed

Full joint spectral-siren fit::

    python examples/spectral_sirens_gwcat_h100.py \
        --mock-dir data/mock_closure --require-gpu \
        --population powerlaw+peak --nlive 1000 --dlogz 0.1 \
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Closure directory containing manifest.json; replaces PE/SELECTION.",
    )
    ...
    p.add_argument(
        ...
        help="Fix population; omit for a joint spectral-siren population fit.",
    )
...
def _resolve_inputs(args):
    ...
    if root is not None:
        if args.pe is not None or args.selection is not None:
            raise ValueError("use either --mock-dir or positional PE/SELECTION, not both")
        pe = mock_file(root, manifest, "gw_events")
        selection = mock_file(root, manifest, "gw_selection")
    else:
        if args.pe is None or args.selection is None:
            raise ValueError("provide PE SELECTION or --mock-dir")
        pe = args.pe.expanduser().resolve()
        selection = args.selection.expanduser().resolve()
    return pe, selection, root, manifest
...
def main(argv: list[str] | None = None) -> int:
    ...
    pe, selection, mock_root, manifest = _resolve_inputs(args)
    ...
    if not args.skip_likelihood_benchmark:
        print_header("Spectral likelihood benchmark")
        ...
        print(f"bind: {benchmark['bind_s']:.3f} s")
        print(
    ...
    if not args.benchmark_only:
        print_header(f"Full spectral inference: {args.sampler}")
        options = dict(
            nlive=args.nlive,
            dlogz=args.dlogz,
            seed=args.seed,
    ...
    print_header("Done")
    print("benchmark-only" if args.benchmark_only else "spectral-siren run complete")
    return 0
