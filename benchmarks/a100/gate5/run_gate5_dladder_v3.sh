...
# v3: Fable exception: core-main dynesty rung 1 gets a 7200 s sampling wall (eager adapter ~0.22 s per call, so
# 20,000 evaluations need ~75 min; the wall is a safety only and must not bind). Phase c5b = the rung-5b compare
# (the v2 driver was stopped between the 5b runs to switch to this copy; the in-flight run finished under v2).
# usage: run_gate5_dladder_v3.sh PHASE...   (a1 a2 c5b b c d)
set -u
...
run() {  # arm rung sampler guard [max_evals] [max_wall_s]
  local arm=$1 rung=$2 sampler=$3 guard=$4 mev=${5:-$MAX_EVALS} mwall=${6:-$MAX_WALL} impl env armlbl refs cap pol=$POLICY
  [ "$mwall" != "$MAX_WALL" ] && pol="$POLICY. EXCEPTION for this run (Fable): max_wall_s $mwall because the core-main dynesty adapter calls the likelihood eagerly at ~0.22 s per call, so 20,000 evaluations need ~75 min; the wall is a safety only"
  ...
  mkdir -p "$cdir"
  local t0=$(date +%s.%N)
  echo "$(date -u +%FT%TZ) start $name (guard $guard cap $cap max_evals $mev max_wall_s $mwall)"
  env -u PYTHONPATH -u JAX_PLATFORMS PYTHONDONTWRITEBYTECODE=1 $R/bin/gpu_run.sh $G5/smi/$name.smi.csv bash -c "source $env && cd $G5 && exec timeout -s TERM -k 300 $TIMEOUT_S python $H/infer_ladder.py --impl $impl --rung $rung --sampler $sampler --pe $PE --sel $SEL --nlive 1000 --dlogz 0.1 --seed 20260924 --max-samples 0 --guard $guard --max-variance $cap --tinyns-preset recommended --sel-batch none --pe-block none --out $out --device gpu --cache-dir $cdir --cache-mode cold --smi-log $G5/smi/$name.smi.csv --label $name --arm $armlbl --max-evals $mev --max-wall-s $mwall --policy-note '$pol' $pr" > $G5/logs/$name.stdout 2> $G5/logs/$name.stderr
  local rc=$?
...
for PHASE in "$@"; do
 case $PHASE in
  ...
  c5b) cmp $(nm legacy 5b tinyns soft) $(nm main 5b tinyns soft) tinyns_r5b_soft_legacy_vs_main;;
  c)  run main 1 dynesty soft 20000 7200
