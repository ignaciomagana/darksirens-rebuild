...
class CountingLikelihood:
    ...
    def __init__(self, fn):
        ...
        self.phase = "init"
        self.counts = {}
        # eager calls whose value is not finite (the hard guard's -inf wall, or NaN), per
        # (phase, kind); read after block_until_ready, the sampler still receives `out` itself
        self.nonfinite = {}
    ...
    def nonfinite_table(self):
        return {f"{p}:{k}": v for (p, k), v in sorted(self.nonfinite.items())}
...
def main(argv=None):
    ...
    record["likelihood_calls"] = {"by_phase_kind": lk.table(),
                                  ...
                                  "nonfinite": {
                                      "by_phase_kind": lk.nonfinite_table(),
                                      "sampling_eager_total": lk.nonfinite.get(("sampling", "eager"), 0),
                                      "note": ("eager likelihood calls returning -inf or NaN (the hard "
                                               "selection guard's wall), counted by the wrapper after "
                                               "block_until_ready; dynesty: preflight draws + initial "
                                               "live-point draws + rejected proposals; observation only")}}
    if a.smi_log:
